...
db.set(`kufur-engel_${id}`, newSettings['küfürEngel'])
}
if (!newSettings['küfürEngel']) {
db.delete(`kufur-engel_${id}`)
}

if (newSettings['linkEngel'] === 'aktif') {
db.set(`linkE_${id}`, newSettings['linkEngel']) //db.fetch() için içindeki linkE yi kendi veritabanınızdaki link engelleme sisteminin ism ini yaz
}
      
if (!newSettings['linkEngel']) {
db.delete(`linkE_${id}`)//buraya da aynı şekilde
}
...
db.set(`guvenlik${id}`, newSettings['guvenlik']);
} 
if (newSettings['log']) {
