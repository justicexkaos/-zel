...
module.exports = (client) => {
  ...
  const bilgiler = {
    oauthSecret: "OAUTH2 SECRET KEY", 
    callbackURL: `https://siteniz.glitch.me/callback`,
    domain: `https://siteniz.glitch.me`
  };
...
//    client.channels.get("592809400469684226").send(`**${client.users.get(req.user.id).tag}** adlı kullanıcı Web Paneline Discord hesabıyla giriş yaptı!`)
  ...
  app.get("/anasayfa", (req, res) => {
    ...
    yukle(res, req, "anasayfa.ejs", {sunucu, kullanici, kanal});
  });
    ...
    yukle(res, req, "sayfa-komutlar.ejs", {sunucu, kullanici, kanal});
  });
  ...
  app.get("/panel/:sunucuID/yonet", girisGerekli, async(req, res) => {
    ...
    yukle(res, req, "sayfa-ayarlar.ejs", {guild, sunucu});
...
if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
  ...
  app.get("/panel/:sunucuID/otorol/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`autoRol_${req.params.sunucuID}`) === false) return res.json({"hata": "Otorol adlı ayar "+client.guilds.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
    client.ayar.delete(`autoRol_${req.params.sunucuID}`)
    res.redirect(`/panel/${req.params.sunucuID}/otorol`);
  });
  
   app.get("/panel/:sunucuID/otoRK/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`otorolKanal_${req.params.sunucuID}`) === false) return res.json({"hata": "Otorol kanalı   "+client.guilds.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
    client.ayar.delete(`otorolKanal_${req.params.sunucuID}`)
    res.redirect(`/panel/${req.params.sunucuID}/otorol`);
  });
...
if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
  ...
  app.get("/panel/:sunucuID/mlogk/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`mlog_${req.params.sunucuID}`) === false) return res.json({"hata": "Mod Log kanalı "+client.guilds.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
    client.ayar.delete(`mlog_${req.params.sunucuID}`)
    res.redirect(`/panel/${req.params.sunucuID}/genel`);
  });
  
  app.get("/panel/:sunucuID/logk/sifirla", girisGerekli, (req, res) => {
  ...
  app.get("/panel/:sunucuID/guvenlik/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`guvenlik${req.params.sunucuID}`) === false) return res.json({"hata": "Güvenlik kanalı "+client.guilds.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
    client.ayar.delete(`guvenlik${req.params.sunucuID}`)
    res.redirect(`/panel/${req.params.sunucuID}/genel`);
  });
...
res.redirect("/panel/"+req.params.guildID+"/yonet");
});
...
res.redirect("/panel/"+req.params.guildID+"/genel");
  ...
  app.post("/panel/:sunucuID/yonet", girisGerekli, (req, res) => {
    const sunucu = client.guilds.get(req.params.sunucuID);
    const g = client.guilds.get(req.params.sunucuID);
    const guild = client.guilds.get(req.params.sunucuID);
    if (!sunucu) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
    const isManaged = sunucu && !!sunucu.member(req.user.id) ? sunucu.member(req.user.id).permissions.has("MANAGE_GUILD") : false;
    if (!isManaged && !req.session.isAdmin) return res.json({"hata":"Bu sunucuda Sunucuyu Yönet iznin bulunmuyor. Bu yüzden bu sayfaya erişim sağlayamazsın."});
  ...
  app.get("/botuekle", (req, res) => {
    res.redirect(`https://discordapp.com/oauth2/authorize?client_id=609476251727560749&scope=bot&permissions=8`);
  });
  
  
  app.get("/destek", (req, res) => {
    res.redirect(`https://discord.gg/J9E5FJW`);
  });
